...
const updateCategory = async (data,categoryId) => {
    try {
        const response = await axios.post(`${baseURL}/category/${categoryId}`, data, { headers: getAuthHeaders()})
        return response
    } catch (error) {
        return {error: error.message}
    }
...
const deleteCategory = async (categoryId) => {
    try {
        const response = await axios.post(`${baseURL}/category/${categoryId}`, { headers: getAuthHeaders()})
        return response 
    } catch (error) {
        return {error: error.message}
    }
...
const getAllCategoriesByType = async (type) => {
  try {
    const url = type ? `${baseURL}/category?type=${type}` : `${baseURL}/category`
